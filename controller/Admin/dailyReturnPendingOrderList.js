...
const dailyReturnPendingOrderList = async (req, res, next) => {
    try {
        const junction = await rel_order_order_details_user_agent_vendor.findAll({
            attributes: ['id'],
            where: {
                [Op.and]: [{ status: "pending" }, { orderType: 'return' }, { date: DataTypes.NOW }]
            },
            order: [['id', 'DESC']],
            include: [{
                model: orderDetails,
...
module.exports = dailyReturnPendingOrderList;

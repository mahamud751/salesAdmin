...
const getRemainderAdminProductStockQty = async (req, res, next) => {
    try {
        const junction = await product.findAll({
            where: { [Op.and]: [{ productOwner: 'Admin' }, { productType: 'Regular' }] },
            order: [['id', 'DESC']],
            include: [{
                model: category,
                attributes: ['title'],
...
module.exports = getRemainderAdminProductStockQty;

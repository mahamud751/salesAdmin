...
import { storeAllReturnSuccessListOfProduct } from "../../atom/listOfAllPending";
import useScript from "../../commonFunction/ReloadJs";

const AllSuccessProductOrderReturn = (props) => {
  useScript("/assets/js/app.js");

  const getAllSuccessProductOrderReturn = props.data;
  console.log(getAllSuccessProductOrderReturn);
  const [cancelOrder, updateAllSuccessProductOrderReturn] = useRecoilState(storeAllReturnSuccessListOfProduct);

  useEffect(() => {
    updateAllSuccessProductOrderReturn(getAllSuccessProductOrderReturn);
  }, [updateAllSuccessProductOrderReturn]);
  ...
  //     });
  // };
  const AllSuccessProductOrderReturn = useRecoilValue(storeAllReturnSuccessListOfProduct);
            ...
            <div className="card-body">
              <div className="datatable-wrapper table-responsive">
                <table id="datatable" className="display compact table table-striped table-bordered">
                  ...
                  <tbody>
                    {AllSuccessProductOrderReturn.map((item, index) => (
                      <tr key={index}>
                        <td>{index + 1}</td>
                        {/* <td>
...
export async function getServerSideProps(context) {
  const { data } = await axios.get(process.env.API_URL + "/GetAllReturnSuccessOrderList");
  // console.log(data)
  if (!data) {
    return {
      notFound: true,
...
export default AllSuccessProductOrderReturn;

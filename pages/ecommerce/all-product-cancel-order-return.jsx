...
import { storeAllReturnCancelListOfProduct } from "../../atom/listOfAllPending";
import useScript from "../../commonFunction/ReloadJs";

const AllCancelProductOrderReturn = (props) => {
  useScript("/assets/js/app.js");

  const getCancelProductList = props.data;
  console.log(getCancelProductList);
  const [cancelOrder, updateCancelOrder] = useRecoilState(storeAllReturnCancelListOfProduct);
  ...
  //     });
  // };
  const allCancelOrdersReturn = useRecoilValue(storeAllReturnCancelListOfProduct);
            ...
            <div className="card-body">
              <div className="datatable-wrapper table-responsive">
                <table id="datatable" className="display compact table table-striped table-bordered">
                  ...
                  <tbody>
                    {allCancelOrdersReturn.map((item, index) => (
                      <tr key={index}>
                        <td>{index + 1}</td>
                        {/* <td>
...
export async function getServerSideProps(context) {
  const { data } = await axios.get(process.env.API_URL + "/GetAllReturnCancelOrderList");
  // console.log(data)
  if (!data) {
    return {
      notFound: true,
...
export default AllCancelProductOrderReturn;

...
import { storeAllPendingListOfProduct } from "../../atom/listOfAllPending";
import useScript from "../../commonFunction/ReloadJs";

const AllPendingProductOrder = (props) => {
  useScript("/assets/js/app.js");

  const allPendingOrder = props.data;
  console.log(allPendingOrder);
  const [cancelOrder, updatePendingOrder] = useRecoilState(storeAllPendingListOfProduct);

  useEffect(() => {
    updatePendingOrder(allPendingOrder);
  }, [updatePendingOrder]);
  ...
  //     });
  // };
  const allPendingOrders = useRecoilValue(storeAllPendingListOfProduct);
            ...
            <div className="card-body">
              <div className="datatable-wrapper table-responsive">
                <table id="datatable" className="display compact table table-striped table-bordered">
                  ...
                  <tbody>
                    {allPendingOrders.map((item, index) => (
                      <tr key={index}>
                        <td>{index + 1}</td>
                        {/* <td>
...
export async function getServerSideProps(context) {
  const { data } = await axios.get(process.env.API_URL + "/GetAllPendingOrderList");
  // console.log(data)
  if (!data) {
    return {
      notFound: true,
...
export default AllPendingProductOrder;

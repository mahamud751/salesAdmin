...
const DailySuccessOrderReturn = (props) => {
  useScript("/assets/js/app.js");

  const getReturnSuccessProductList = props.data;
  console.log(getReturnSuccessProductList);
  const [returnPendingOrder, updateReturnSuccessOrder] = useRecoilState(storeReturnSuccessListOfProduct);

  useEffect(() => {
    updateReturnSuccessOrder(getReturnSuccessProductList);
  }, [updateReturnSuccessOrder]);
  ...
  const returnSuccessOrders = useRecoilValue(storeReturnSuccessListOfProduct);
  console.log(returnSuccessOrders);
  return (
    <div>
      <div className="row">
            ...
            <div className="card-body">
              <div className="datatable-wrapper table-responsive">
                <table id="datatable" className="display compact table table-striped table-bordered">
                  ...
                  <tbody>
                    {returnSuccessOrders.map((item, index) => (
                      <tr key={index}>
                        <td>{index + 1}</td>
                        {/* <td>
...
export default DailySuccessOrderReturn;

...
import { storeAllProcessListOfProduct } from "../../atom/listOfAllPending";
import useScript from "../../commonFunction/ReloadJs";

const AllProcessProductOrder = (props) => {
  useScript("/assets/js/app.js");

  const getProcessProductList = props.data;
  console.log(getProcessProductList);
  const [cancelOrder, updateProcessOrder] = useRecoilState(storeAllProcessListOfProduct);

  useEffect(() => {
    updateProcessOrder(getProcessProductList);
  }, [updateProcessOrder]);
  ...
  //     });
  // };
  const allProcessOrder = useRecoilValue(storeAllProcessListOfProduct);
            ...
            <div className="card-body">
              <div className="datatable-wrapper table-responsive">
                <table id="datatable" className="display compact table table-striped table-bordered">
                  ...
                  <tbody>
                    {allProcessOrder.map((item, index) => (
                      <tr key={index}>
                        <td>{index + 1}</td>
                        {/* <td>
...
export async function getServerSideProps(context) {
  const { data } = await axios.get(process.env.API_URL + "/GetAllProcessOrderList");
  // console.log(data)
  if (!data) {
    return {
      notFound: true,
...
export default AllProcessProductOrder;

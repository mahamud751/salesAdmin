...
import { storeReturnPendingListOfProduct } from "../../atom/listOfPendingOrder";
import useScript from "../../commonFunction/ReloadJs";

const DailyPendingProductOrderReturn = (props) => {
  useScript("/assets/js/app.js");

  const getReturnPendingProductList = props.data;
  console.log(getReturnPendingProductList);
  const [returnPendingOrder, updateReturnPendingOrder] = useRecoilState(storeReturnPendingListOfProduct);

  useEffect(() => {
    updateReturnPendingOrder(getReturnPendingProductList);
  }, [updateReturnPendingOrder]);
  ...
  const returnPendingOrders = useRecoilValue(storeReturnPendingListOfProduct);
  console.log(returnPendingOrders);
  return (
    <div>
      <div className="row">
            ...
            <div className="card-body">
              <div className="datatable-wrapper table-responsive">
                <table id="datatable" className="display compact table table-striped table-bordered">
                  ...
                  <tbody>
                    {returnPendingOrders.map((item, index) => (
                      <tr key={index}>
                        <td>{index + 1}</td>
                        {/* <td>
...
export async function getServerSideProps(context) {
  const { data } = await axios.get(process.env.API_URL + "/DailyReturnPendingOrderList");
  // console.log(data)
  if (!data) {
    return {
      notFound: true,
